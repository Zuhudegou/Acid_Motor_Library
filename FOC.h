...
// 电机控制核心函数文件声明
#include "Cogging.h"                  // Cogging离线标定抗齿槽算法
#include "DOB.h"                      // DOB超螺旋滑模扰动观测器
#include "Feedforward.h"              // Feedforward前馈环节
#include "Filter.h"                   // Filter低通滤波器和陷波器
#include "Hall.h"                     // Hall三霍尔信号处理
#include "HFI.h"                      // HFI(无感)高频正弦波注入
#include "Identify.h"                 // Identify电机参数辨识
#include "Ladrc.h"                    // Ladrc线自抗扰控制
#include "LTD.h"                      // LTD最速控制(平滑阶跃信号)
#include "MotorStatus.h"              // MotorStatus电机状态机
#include "MIT_Protocol.h"             // MIT协议阻抗控制
#include "NLFO.h"                     // NLFO(无感)非线性磁链观测
#include "NSD.h"                      // NSD电机极性辨识算法
#include "Optimize.h"                 // Optimize电机优化算法
#include "PID.h"                      // PID传统闭环控制
#include "PLL.h"                      // PLL角度跟踪“锁相环”
#include "printf.h"                   // printf通信调试
#include "SMC.h"                      // SMC传统滑模控制
#include "SMO.h"                      // SMO(无感)静止坐标系下的滑模观测器
#include "SPWM.h"                     // SPWM零序分量注入法的PWM调制
#include "STA.h"                      // STA超螺旋简化滑模控制
#include "SVPWM.h"                    // SVPWM七段式空间矢量PWM调制
/* USER CODE END Includes */

// ╔═════════════════════════════════════════════════════════╗
// ║                       开环控制模式                       ║
// ╚═════════════════════════════════════════════════════════╝
#define MODE_VF_OPENLOOP        0x00        // VF压频比开环(FOC.foc.Target_Speed,FOC.foc.Uq_in)
#define MODE_IF_OPENLOOP        0x01        // IF流频比开环(FOC.foc.Target_Speed,FOC.foc.Target_Iq)
// ╔═════════════════════════════════════════════════════════╗
// ║                   闭环控制模式（有传感器）                ║
// ╚═════════════════════════════════════════════════════════╝
#define MODE_Voltag_OPEN        0x02        // 电压开环(FOC.foc.Uq_in)
#define MODE_Current_SINGLE     0x03        // 电流单闭环(FOC.foc.Target_Iq)
#define MODE_VelCur_DOUBLE      0x04        // 速度-电流串级闭环(FOC.foc.Target_Speed)
#define MODE_PosVelCur_THREE    0x05        // 位置-速度-电流三环(FOC.foc.Target_Pos)
// ╔═════════════════════════════════════════════════════════╗
// ║                   霍尔控制模式（有传感器）                ║
// ╚═════════════════════════════════════════════════════════╝
#define MODE_Sensor_Hall        0x06        // 有感霍尔_转速环(FOC.foc.Target_Speed)
// ╔═════════════════════════════════════════════════════════╗
// ║                   无感控制模式（无传感器）                ║
// ╚═════════════════════════════════════════════════════════╝
#define MODE_Sensorless_HFI     0x07        // 高频注入_转速环(FOC.foc.Target_Speed)
#define MODE_Sensorless_SMO     0x08        // 滑模观测_转速环(FOC.foc.Target_Speed)
#define MODE_Sensorless_NLFO    0x09        // 非线性磁链_转速环(FOC.foc.Target_Speed)
#define MODE_Sensorless_HS      0x0A        // 高频滑模结合_转速环(FOC.foc.Target_Speed)
#define MODE_Sensorless_HN      0x0B        // 高频磁链结合_转速环(FOC.foc.Target_Speed)
#define MODE_Sensorless_AS      0x0C        // 霍尔滑模结合_转速环(FOC.foc.Target_Speed)
#define MODE_Sensorless_AN      0x0D        // 霍尔磁链结合_转速环(FOC.foc.Target_Speed)
// ╔═════════════════════════════════════════════════════════╗
// ║                 DEBUG模式（有感,外载“无感观测器”）        ║
// ╚═════════════════════════════════════════════════════════╝
#define MODE_Debug_HFI          0x0E        // HFI测试_转速环(FOC.foc.Target_Speed)
#define MODE_Debug_SMO          0x0F        // SMO测试_转速环(FOC.foc.Target_Speed)
#define MODE_Debug_NLFO         0x10        // NLFO测试_转速环(FOC.foc.Target_Speed)
#define MODE_Debug_HS           0x11        // HFI切SMO_转速环(FOC.foc.Target_Speed)
#define MODE_Debug_HN           0x12        // HFI切NLFO_转速环(FOC.foc.Target_Speed)


// +---------------------------------------------------------+
// |                    控制器Ctrl宏定义                      |
// +---------------------------------------------------------+
#define Control_PID             0x00        // PID闭环控制PID
#define Control_LADRC           0x01        // LADRC线自抗扰LADRC
#define Control_SMC             0x02        // SMC传统滑模SMC
#define Control_STA             0x03        // STA简易超螺旋STA
// +---------------------------------------------------------+
// |                    滤波器Filter定义                      |
// +---------------------------------------------------------+
...
typedef struct{    
    ...
    // 如果有电机电压预警，电机正常运行...经过Low_Loop()的VBUS_watchdog_limit此运行周期
    ...
    // 如果有驱动器温度预警，电机正常运行...经过Low_Loop()的Temp_watchdog_limit此运行周期
    ...
    // 如果有电机过流预警，电机正常运行...经过Low_Loop()的DQcur_watchdog_limit此运行周期
...
/**
 * @description: FOC系统核心结构体定义
 * @reminder: （FOC总结构体成员）
 * @struct {FOC_System_STRUCT}
 */
typedef struct{
    // ============================= ①电机标识位 =================================
