...
// ========================= 达妙MIT协议配置 =============================
// 达妙MIT阻抗控制需要电流单闭环模式(Define_Run_Mode = 3)
#ifndef MIT_PROTOCOL_ENABLE
#define MIT_PROTOCOL_ENABLE 1
#endif

// CAN_ID：基础控制帧ID，范围0x00~0xFF；其他模式会自动加0x100/0x200/0x300
#ifndef MIT_CAN_ID
#define MIT_CAN_ID 1U
#endif

// MST_ID：本驱动器向上位机发送反馈帧所使用的11位标准帧ID
#ifndef MIT_MASTER_ID
#define MIT_MASTER_ID 0x11U
#endif

// 上电默认达妙控制模式：1=MIT，2=位置速度，3=速度，4=力位混合
#define MIT_DEFAULT_CONTROL_MODE 1U

// 串口仅用于切换控制模式，不再发送JustFloat或修改控制目标
#define UART_MODE_SWITCH_ONLY    1

// 默认采用DM4310量程；必须与上位机中所选型号的P_MAX/V_MAX/T_MAX完全一致。
...
#define MIT_POSITION_MIN       (-MIT_P_MAX)
#define MIT_POSITION_MAX       (MIT_P_MAX)
#define MIT_VELOCITY_MIN       (-MIT_V_MAX)
#define MIT_VELOCITY_MAX       (MIT_V_MAX)
#define MIT_TORQUE_MIN         (-MIT_T_MAX)
#define MIT_TORQUE_MAX         (MIT_T_MAX)

// 输出轴力矩常数KT_OUT，单位N.m/A。
// 设为0时使用1.5 * 极对数 * 磁链作为默认值；实机必须标定后填写。
#define MIT_KT_OUT             (0.0f)

// 超过此时间没有收到控制帧则Iq清零并停止电机，单位ms
#define MIT_COMMAND_TIMEOUT_MS 100U
#define MIT_TIMEOUT_STOP       1

// STM32 HAL FDCAN适配。当前纯算法库没有main.h，移植到STM32工程后改为1。
// 达妙CAN为1Mbps、11位标准帧、Classic CAN、8字节、关闭BRS。
#ifndef MIT_FDCAN_HAL_ENABLE
#define MIT_FDCAN_HAL_ENABLE   0
#endif
#define MIT_FDCAN_HAL_HEADER   "main.h"
#define MIT_FDCAN_HANDLE       hfdcan1
#define MIT_FDCAN_FILTER_INDEX 0U
#define MIT_FDCAN_RX_QUEUE_SIZE 4U

#if MIT_PROTOCOL_ENABLE && (Define_Run_Mode != 3)
#error "DaMiao MIT protocol requires Define_Run_Mode = 3"
#endif

#if (MIT_CAN_ID > 0xFFU)
#error "MIT_CAN_ID must be <= 0xFF because DaMiao modes use +0x100/+0x200/+0x300"
#endif

#if MIT_MASTER_ID > 0x7FFU
#error "MIT_MASTER_ID must be an 11-bit standard CAN identifier"
#endif

#if (MIT_DEFAULT_CONTROL_MODE < 1U) || (MIT_DEFAULT_CONTROL_MODE > 4U)
#error "MIT_DEFAULT_CONTROL_MODE must be 1..4"
#endif

#if MIT_FDCAN_RX_QUEUE_SIZE < 2U
#error "MIT_FDCAN_RX_QUEUE_SIZE must be at least 2"
#endif
