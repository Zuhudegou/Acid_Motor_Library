...
static inline void User_Initial_Init(void){
    ...
    // // 启用串口DMA接收
    // HAL_UARTEx_ReceiveToIdle_DMA(&huart1, PrintfBuff, sizeof(PrintfBuff));
...
/**
 * @description: 10.达妙MIT反馈中的转子温度读取接口
 * @reminder: 未单独测量转子温度时保持默认，协议会使用MOS温度代替
 * @return {*}
 */
static inline float User_RotorTemperature_DataGet(void){
    /* Your code for motor rotor temperature here */
    return 0xFF800000;
}

/**
 * @description: 11.达妙MIT保存零点命令的用户持久化接口
 * @reminder: 收到FF FF FF FF FF FF FF FE后调用，可在此写入Flash
 * @param {float} zero_offset 当前机械零点偏置(rad)
 * @return {*}
 */
static inline void User_MIT_SaveZero(float zero_offset){
    (void)zero_offset;
    /* Your code for persisting the MIT mechanical zero offset here */
}

/**
 * @description: 12.用户的通信接口设计
